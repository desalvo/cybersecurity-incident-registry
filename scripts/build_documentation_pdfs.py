...
def _text_lines_from_template(template_name: str) -> list[str]:
    ...
    lines = [
        line.replace("Versione applicativa: 0.6.0-31 · Build: · Autore: .", "Versione applicativa: 0.6.0-31 · Build: 20260530 · Autore: Alessandro De Salvo.")
        for line in lines
    ]
...
def _build_pdf(kind: str, template_name: str, output_name: str, title: str, subtitle: str, callout: str, visuals_by_chapter: dict[str, list[tuple[str, str]]]) -> None:
    ...
    if template_name == "help_en.html":
        lines = _trim_english_preface_after_version_marker(
            lines,
            "Version 0.6.0-31 of the user guide, aligned with build",
        )
    elif template_name == "admin_help_en.html":
        lines = _trim_english_preface_after_version_marker(
            lines,
            "Application version: 0.6.0-31 · Version 0.6.0-31 of the administrator guide.",
        )
...
def _build_brochure() -> None:
    ...
    header = Table(
        [[logo, [
            ...
            Paragraph("Versione applicativa 0.6.0-31 - Registro operativo containerizzato per incidenti cyber, workflow, notifiche, audit e documentazione probatoria.", subtitle),
        ]]],
...
def _build_brochure_en() -> None:
    ...
    header = Table(
        [[logo, [
            Paragraph("Cybersecurity Incident Registry", title),
            Paragraph("Application version 0.6.0-31 - Container-ready operational registry for cyber incidents, workflows, notifications, audit evidence and documentation.", subtitle),
        ]]],
        colWidths=[2.25 * cm, 15.9 * cm],
    )
    header.setStyle(TableStyle([
