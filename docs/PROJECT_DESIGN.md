...
Per i promemoria specifici il criterio funzionale di blocco dell'invio resta soltanto `IncidentReminder.sent_at`. La funzione `_claim_incident_reminder()` non valorizza né interpreta `last_schedule_slot`: il record in `deadline_notification_state` viene usato come claim anti-concorrenza temporaneo per impedire invii simultanei dello stesso promemoria, non come deduplica per periodo o finestra.
