...
Il thread scheduler esegue sia il controllo periodico dei task in scadenza sia il recupero dei promemoria specifici. Per i task periodici la pianificazione resta ancorata agli slot calcolati dalla mezzanotte nella timezone applicativa. Per i promemoria specifici non periodici vengono inviati i record scaduti con `sent_at` nullo, ma prima dell'invio viene registrato un claim persistente in `deadline_notification_state`, così due cicli contemporanei non possono inviare lo stesso promemoria.
