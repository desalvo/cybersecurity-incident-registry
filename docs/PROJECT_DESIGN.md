...
Il plugin `app/plugins/ai_chatbot` espone anche un widget globale integrato nel layout base. Quando `plugin_ai_chatbot_enabled=1` e l’utente è autenticato, `base.html` inserisce il launcher desktop nell’area in basso a destra e il pulsante mobile accanto al menu. Le regole CSS in `app/static/style.css` mantengono il launcher e il pannello desktop spostati a sinistra del logo decorativo `.app-corner-logo`, evitando sovrapposizioni con il logo applicativo; su mobile il pulsante resta nella griglia della topbar e passa a riga dedicata sugli schermi stretti. Il submit AJAX usa l’endpoint `ai_chatbot.widget_ask`, protetto da login e CSRF, e riusa la stessa pipeline di contesto, motore AI, audit e sanitizzazione già usata dalla pagina completa del chatbot. La finestra è richiudibile/iconizzabile lato client senza perdere la pagina corrente.
