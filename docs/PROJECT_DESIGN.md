## Aggiornamento 0.7.0-7 - Campi personalizzati nei Dati Generali e UI incidente

La release 0.7.0-7 build 20260608 aggiunge campi personalizzati configurabili nella sezione Dati Generali degli incidenti specifici. Gli amministratori possono definire campi testo, data/ora, nascosti con rivelazione esplicita e checkbox con descrizione da Admin -> Configurazione generale -> Campi personalizzati; la visibilità di ogni campo è disponibile in Admin -> Layout campi incidenti. La pagina incidente mostra il Riferimento sotto al nome dell'incidente e posiziona l'icona di ritorno a inizio pagina verticalmente al centro delle sezioni successive alla prima.

## Aggiornamento 0.7.0-7 - Bonifica release e stabilizzazione test

La release 0.7.0-7 build 20260608 normalizza i metadati esposti da applicazione, documentazione, README, Docker Compose e manifest Kubernetes. Il pacchetto rimuove file spurii non sorgente e mantiene una struttura distribuibile pulita. Il ciclo di vita degli scheduler e' stato parzialmente separato in `app/route_modules/scheduler_lifecycle.py`, preservando gli entry point di compatibilita' in `app.routes`. La suite `python -m pytest` viene instradata al runner offline-safe a moduli isolati, con plugin pytest esterni disabilitati, scheduler di background spenti in test e uscita deterministica dei processi figlio. Le versioni PDF operative in `docs/` sono rigenerate per la stessa build.

## Aggiornamento 0.7.0-7 - Azioni automatiche condizionate dal percorso workflow

La configurazione **Admin -> Anagrafiche e workflow -> Azioni automatiche pulsanti** presenta ogni pulsante in una sezione verticale collassabile, chiusa per default. Per ogni pulsante operativo registra la label azione da inserire e la condizione di attivazione. La condizione puo' essere `sempre` oppure `solo da sezione aperta tramite step workflow`: nel secondo caso il client marca i form e i link locali di download documento contenuti nella sezione aperta da uno step `Aggiorna sezione` o `Operazione` con il parametro `workflow_update_section_redirect=1`, e il server registra l'azione automatica solo se tale parametro accompagna il submit o la richiesta GET riuscita. Per `Salva tag` la configurazione usa una lista di regole: ogni regola associa univocamente il pulsante a un insieme di tag selezionati via drag and drop e a una specifica label azione; ogni tag può appartenere a una sola regola per lo stesso pulsante, vincolo applicato sia dalla UI sia dalla normalizzazione server-side; quando vengono salvati i tag del documento, il server registra tutte le azioni associate alle regole che intercettano almeno uno dei tag presenti. Per `Upload documenti` la configurazione usa il codice `document_upload` come lista ordinata di regole `{label_id, scope, template_name, notification_tags}`. `template_name` e' opzionale: se la sezione Documenti e' aperta da uno step `Aggiorna sezione` o `Operazione` con `section_target=incident-documents`, `document_generation_enabled=true` e `document_template_name` valorizzato, il server usa la prima regola `document_upload` con lo stesso template, registra l'azione e applica ai documenti caricati i tag selezionati nella regola. Fuori da questo vincolo, o se lo step non richiede un template, usa la prima regola senza template e non applica tag automatici. Per `Scarica documenti` la configurazione usa il codice `document_download` come lista ordinata di regole `{label_id, scope, template_name}`. `template_name` e' opzionale: fuori da un contesto workflow, oppure quando lo step non indica un template, il server usa la prima regola senza template e ignora le regole specifiche per template. Quando invece la sezione Documenti e' aperta da uno step con template documento richiesto, il client nasconde i link diretti nella colonna Documento, mostra solo il pulsante Scarica dei documenti generati da quel template se esiste una regola `document_download` con lo stesso template; senza match non mostra i pulsanti Scarica. La route di download registra l'azione automatica solo usando la prima regola con template uguale a quello dello step workflow corrente. Dopo Upload e Scarica la pagina incidente viene ricaricata sulla sezione Documenti; il pulsante Upload nella sezione Documenti resta disabilitato finché non è stato selezionato almeno un file. Ogni sezione successiva alla prima espone a sinistra, fuori dal riquadro, una icona cliccabile di ritorno a inizio pagina, con dimensioni e spaziatura adattate anche al layout mobile.

## Aggiornamento 0.7.0-7 - Workflow aggiorna sezione e azioni automatiche pulsanti

Il modello `IncidentWorkflowStep` supporta la tipologia protetta `update_section`: lo step apre una sezione del dettaglio incidente e viene considerato completato quando e' presente l'azione associata allo step. La configurazione amministrativa permette inoltre di associare label azione automatiche ai principali pulsanti operativi della form incidente, anche limitandone l'esecuzione ai pulsanti premuti da sezioni aperte tramite workflow.

## Aggiornamento 0.7.0-7 - Struttura documentazione utente e amministrativa
...
## Aggiornamento 0.7.0-7 - Anteprima CC precompilata e svuotamento manuale
...
## Baseline progettuale 0.7.0-7 - build 20260608
...
## Aggiornamento 0.7.0-7 - Pulsanti data/ora locale e azioni notifica
...
## Aggiornamento 0.7.0-7 - Workflow con dipendenze da notifiche e percorso guidato
...
La baseline 0.7.0-7 stabilizza l'applicazione come registro operativo bilingue per incidenti informatici. Include:
...
Il bootstrap iniziale inserisce le label predefinite solo su database non ancora popolato. Quando l'applicazione si avvia su un database esistente, eventuali valori predefiniti mancanti non vengono ricreati automaticamente; l'amministratore può usare il pulsante **Reinserisci valori predefiniti mancanti** in `Admin -> Liste configurabili`, che aggiunge solo i default assenti per gravità, dati interessati, categorie incidente e label azioni.
...
La build 20260608 aggiunge il modulo `app/security.py`, inizializzato da `create_app()` prima della registrazione delle blueprint. Il modulo applica una baseline production-ready senza introdurre dipendenze esterne obbligatorie:

- validazione CSRF per tutti i metodi mutativi (`POST`, `PUT`, `PATCH`, `DELETE`), con token per sessione e confronto costante tramite `secrets.compare_digest`; la variabile `CIR_DISABLE_CSRF=1` disabilita la validazione solo in locale/test ed è rifiutata in produzione;
...
- cookie di sessione e remember-cookie `HttpOnly`, `SameSite=Lax` e `Secure` quando `SESSION_COOKIE_SECURE=1`; `CIR_PRODUCTION=1` non forza più il flag `Secure`, per consentire login CSRF anche in Docker Compose HTTP con `SESSION_COOKIE_SECURE=0`;
...
- `%RECOMMENDATIONS%`: lista delle raccomandazioni da fornire agli interessati, calcolata dalla stessa origine dati del campo `recommendations` usato per la compilazione automatica dei moduli
...
## Aggiornamento 0.7.0-7 - Identità utente composta
...
### Aggiornamento 0.7.0-7 - Visualizzazione provider nel tipo login SSO
...
## Aggiornamento 0.7.0-7 - Placeholder notifiche manuali e link incidente esplicito
...
La tabella `ExternalRecipient` contiene nome, email e note della rubrica condivisa. I campi destinatario/CC delle notifiche manuali usano questa rubrica come suggerimento; durante l’invio, nuove email digitate manualmente non vengono inserite nella rubrica. La rubrica è amministrabile da menu Admin/Impostazioni, include il comando protetto `Rimuovi tutto` ed è inclusa nel full export/import.


### Tag multipli dei documenti per preselezione allegati notifiche

Il modello `Document` contiene il campo `notification_tags`, una lista compatta di codici di tipo notifica associati al documento. Nel dettaglio incidente la sezione Documenti espone un pulsante `Scarica` per ogni documento e una palette dei tipi notifica con drop-zone per ogni documento: l’utente con permesso di scrittura trascina il tipo sulla drop-zone, può rimuovere il tag e salva la configurazione. La route `update_document_notification_tags` valida i codici rispetto a `NotificationType` e aggiorna solo documenti appartenenti a incidenti visibili all’utente.
...
Metadati runtime aggiornati: versione `0.7.0-7`, build `20260608`.

## Aggiornamento 0.7.0-7 - Gestione rubrica destinatari esterni per utenti writer
...
### Aggiornamento 0.7.0-7 - Workflow interattivo nella pagina incidente
...
## Aggiornamento 0.7.0-7 - Ricerca nella rubrica destinatari esterni
...
## Aggiornamento 0.7.0-7 - Correzione sequence audit nelle operazioni utenti
...
## Aggiornamento 0.7.0-7 - Full import con ricostruzione dello schema database
...
### Correzione sequence utenti 0.7.0-7
...
## Aggiornamento 0.7.0-7 - Riallineamento sequence PostgreSQL generalizzato
...
## Aggiornamento 0.7.0-7 - Riferimento incidente obbligatorio
...
## Versione 0.7.0-7 - Workflow condizionali e provisioning utenti
...
## Versione 0.7.0-7 - Hardening sequence step workflow
...
## Versione 0.7.0-7 - Estensione workflow di default
...
## Aggiornamento 0.7.0-7 - Frecce negli step workflow e versione applicativa

La pagina del singolo incidente visualizza gli step delle operazioni previste con una freccia di sequenza quando il workflow è ordinato tramite il campo `position`. Il template `incident_detail.html` aggiunge l'indicatore tra due step consecutivi e `style.css` ne definisce il layout responsive. I metadati runtime dell'applicazione usano la versione normalizzata `0.7.0-7` e il build `20260608`, visibili in Info → Applicazione e configurabili tramite `APP_VERSION` e `APP_BUILD`.

## Aggiornamento 0.7.0-7 - Cancellazione incidenti con stati deadline collegati
...
## Version 0.7.0-7 - Procedural warnings based on required workflow steps
...
## Versione 0.7.0-7 - Operazioni automatiche sulle label azione
...
## Version 0.7.0-7 - Automatic operations on action labels
...
## Versione 0.7.0-7 - Leggibilità label azioni
...
## Version 0.7.0-7 - Action-label readability
...
## Versione 0.7.0-7 - Controllo globale sui task
...
## Version 0.7.0-7 - Global task check
...
## Versione 0.7.0-7 - Condizioni workflow estese
...
## Version 0.7.0-7 - Extended workflow conditions
...
## Versione 0.7.0-7 - Pulizia versione e documentazione

La versione applicativa esposta da `APP_VERSION` e dalla pagina Info è normalizzata a `0.7.0-7`, con build `20260608`. Le guide utente e amministrativa sono mantenute come documentazione operativa dello stato corrente: non devono contenere banner di versione né blocchi di changelog. Le variazioni cronologiche sono centralizzate in `CHANGELOG.txt` e nelle pagine `release_notes.html` / `release_notes_en.html`.

## Version 0.7.0-7 - Version and documentation cleanup

The application version exposed by `APP_VERSION`, the Info page and the guides is normalized to `0.7.0-7`, with build `20260608`. User and administrator guides are maintained as operational documentation of the current state and must not contain changelog blocks. Chronological changes are centralized in `CHANGELOG.txt` and in `release_notes.html` / `release_notes_en.html`.

## Versione 0.7.0-7 - Riallineamento guide operative
...
## Version 0.7.0-7 - Operational guide alignment
...
## Aggiornamento 0.7.0-7 - Bonifica criticità progettuali
...
## Aggiornamento 0.7.0-7 - Scheduler notifiche seriale e anti-duplicazione mail
...
## Aggiornamento 0.7.0-7 - Stato notifiche schedulate e cambio password solo locale
...
## Aggiornamento 0.7.0-7 - Audit degli incidenti saltati dallo scheduler notifiche
...
## Aggiornamento 0.7.0-7 - Audit robusto nel controllo manuale e promemoria senza slot
...
## Aggiornamento 0.7.0-7 - Controllo manuale dei promemoria specifici e audit dettagliato
...
## Aggiornamento 0.7.0-7 - Risoluzione destinatari dei promemoria specifici
...
## Aggiornamento 0.7.0-7 - Diagnostica controllo manuale promemoria e destinatari deadline
...
## Aggiornamento 0.7.0-7 - Promemoria specifici senza blocco da presa in carico
...
Per evitare invii multipli senza introdurre slot o claim funzionali, su PostgreSQL viene acquisito un lock di riga sul record `IncidentReminder` tramite `with_for_update()`. Un eventuale ciclo concorrente attende la transazione in corso e rivaluta `sent_at` dopo il commit; di conseguenza non viene più prodotto lo skip “già preso in carico”, ma al massimo lo skip “già marcato come inviato” quando il primo ciclo ha completato l'invio e valorizzato `sent_at`.

Il record `deadline_notification_state` con `notification_type='incident_reminder'` viene aggiornato solo come diagnostica per la pagina amministrativa e per i riepiloghi recenti. Non partecipa alla decisione di invio, non contiene slot/finestra per i promemoria specifici e un errore di aggiornamento dello stato diagnostico non deve impedire l'invio del promemoria.

## Stato servizi e cicli scheduler

La pagina `Admin → Stato` espone una vista diagnostica dei servizi applicativi. La route `admin_status()` usa `scheduler_service_status()` per raccogliere stato del thread notifiche, stato dello scheduler backup, fuso orario applicativo, heartbeat, intervallo di poll, schedule configurata per i task in scadenza, contatori dei promemoria specifici e risultato degli ultimi cicli.

Il thread avviato da `start_deadline_notification_scheduler()` non richiama più in modo accoppiato i soli controlli deadline. La funzione `run_scheduler_services_cycle()` esegue due sotto-cicli indipendenti: `run_deadline_notification_check()` per i task con tempo massimo e `process_due_incident_reminders()` per i promemoria specifici. Ogni sotto-ciclo ha gestione autonoma delle eccezioni e registra uno snapshot compatto nelle impostazioni `scheduler_status_deadline_notifications` e `scheduler_status_incident_reminders`, aggiornando anche `scheduler_last_heartbeat_at`.

Questa separazione garantisce che i promemoria specifici scaduti e non ancora inviati vengano controllati ad ogni ciclo dello scheduler anche se le notifiche periodiche dei task in scadenza sono disabilitate, in attesa del primo slot cron o in errore. Il criterio funzionale dei promemoria specifici resta esclusivamente `incident_reminder.sent_at`; gli stati in `deadline_notification_state` rimangono diagnostici e non introducono slot, finestre o prese in carico bloccanti.
...
## Aggiornamento 0.7.0-7 - Scheduler, URL e stato servizi
...
### Aggiornamento 0.7.0-7 - Contesto applicativo dei thread scheduler
...
## Aggiornamento 0.7.0-7 - Sezioni collassabili nel dettaglio incidente
...
## Aggiornamento 0.7.0-7 - Sezioni incidente e workflow guidato
...
## Aggiornamento 0.7.0-7 - Tag documenti nel dettaglio incidente
...
## Aggiornamento 0.7.0-7 - CC predefinito notifiche utente
...
Il campo **E-mail Destinatario** rappresenta l’indirizzo predefinito utilizzabile dai template di notifica manuale. La sua sintassi viene sempre validata lato server nelle form di creazione/modifica incidente e nei modelli incidente; sono accettati solo singoli indirizzi e-mail validi. Se viene valorizzato, deve essere presente almeno un nome associato nel campo **Riferimento** o **Destinatario**. Al salvataggio dell’incidente o del modello, l’e-mail non viene mai inserita automaticamente nella rubrica esterna: la rubrica resta aggiornata solo tramite gestione dedicata o import esplicito.
...
### Aggiornamento 0.7.0-7 - Submit diretto dei destinatari manuali
...
## Aggiornamento 0.7.0-7 - Campi manuali acquisiti al submit notifiche
...
## Aggiornamento 0.7.0-7 - Precedenza CC manuale vuoto

Nelle notifiche manuali, quando il template consente la modifica del campo CC, il valore digitato manualmente resta prevalente; se il campo CC viene svuotato dall’operatore, tale valore vuoto annulla il default del template e la mail viene inviata senza CC. Dalla versione 0.7.0-7 l’anteprima precompila nuovamente il campo CC con il valore di default del template, quando presente, lasciando comunque al submit la priorità al valore manuale effettivo.

## Aggiornamento 0.7.0-7 - Abilitazione esplicita del CC in anteprima
...
## Aggiornamento 0.7.0-7 - Markdown e colori negli step workflow

I riquadri degli step nella sezione **Operazioni previste** renderizzano ora la descrizione operativa con il filtro Jinja `workflow_markdown`. Il rendering supporta Markdown sicuro per titoli, elenchi, grassetto, corsivo, codice inline, link Markdown, URL automatici, colori, dimensioni font controllate e pulsanti link `{button:Etichetta|URL}`. Il testo colorato usa una sintassi controllata `{color:nome}testo{/color}` o `{color:#RRGGBB}testo{/color}`; la dimensione usa `{size:large}testo{/size}` o valori pixel ammessi come `{size:14px}testo{/size}`. L'HTML libero resta escapato. Il template `incident_detail.html` usa contenitori `workflow-markdown` sia per gli step sia per gli avvisi procedurali; questi ultimi mostrano anche il nome del task associato. `style.css` definisce la resa compatta dentro le card del workflow e negli avvisi. La documentazione utente e amministrativa include esempi di Markdown, colori e dimensioni.

## Aggiornamento 0.7.0-7 - Tag notifica configurabili sui template modulo
...
## Aggiornamento 0.7.0-7 - Rimozione tag dai template modulo
...
### 0.7.0-7 - Rischio per diritti e libertà e pending actions condizionate
...
### 0.7.0-7 - Risk to rights and freedoms and conditioned pending actions
...
Il placeholder delle raccomandazioni nei template manuali è `%RECOMMENDATIONS%`; `%RECOMMENDATION%` resta solo come valore storico da migrare automaticamente nei record `notification_template`. Il placeholder `%APP_INFO%` viene calcolato da `current_app.config["APP_INFO"]` e inserisce nome applicazione, versione e build.

La costruzione del campo `consequences` è centralizzata in `app/consequences.py`. Le regole automatiche leggono categorie, tipi di dato e flag di rischio diritti/libertà, sono configurabili tramite `Setting.consequence_rules_json` e sono esposte nella pagina Admin → Altre configurazioni come regole dinamiche modificabili: testo, abilitazione, modalità tutte/una condizione, condizioni su categoria, tipo dato, gravità, stato, rischio diritti/libertà e campi testuali. Le regole possono essere aggiunte o rimosse senza modificare il codice. Le conseguenze esplicite delle azioni (`Action.consequence_text`) vengono sempre aggiunte alla lista risultante, invece di sostituire le conseguenze automatiche.


## Import/export mirato dei workflow

Il menu **Admin → Flussi operativi incidenti** espone un import/export mirato delle definizioni di workflow. L’export produce un JSON con formato `cybersecurity-incident-registry.workflow.v1` e contiene il workflow selezionato, gli step ordinati, condizioni, label referenziate (`category`, `action_label`, `severity`, `data_type`), tipi di notifica richiesti, template di notifica del tipo collegato e template PDF eventualmente associati ai template di notifica. Prima del download viene mostrata la struttura completa e viene richiesta conferma.

L’import è sempre in due fasi: caricamento e preview, poi applicazione. La preview calcola le differenze rispetto al database corrente per label, tipi notifica, template di notifica, template modulo e step workflow. Ogni sovrascrittura richiede una checkbox dedicata; gli elementi nuovi vengono creati automaticamente, mentre gli elementi esistenti non selezionati restano invariati. Questo permette di trasferire un singolo workflow tra ambienti senza dover usare il full export/import e senza sovrascrivere configurazioni locali non confermate.

## Plugin AI Chatbot

È stato introdotto il plugin **AI Chatbot**, isolato in `app/plugins/ai_chatbot/` rispetto al core applicativo. Il plugin registra un blueprint dedicato e mantiene in file separati le interfacce dei motori AI sotto `app/plugins/ai_chatbot/engines/`.
...
Il plugin `app/plugins/ai_chatbot` espone anche un widget globale integrato nel layout base. Quando `plugin_ai_chatbot_enabled=1` e l’utente è autenticato, `base.html` inserisce il launcher desktop nell’area in basso a destra e il pulsante mobile accanto al menu. Le regole CSS in `app/static/style.css` mantengono il launcher e il pannello desktop preferibilmente in basso a destra sopra il logo decorativo `.app-corner-logo`. La funzione client-side `adjustWidgetPosition()` in `app/static/app.js` ricalcola `--ai-chatbot-safe-bottom` e `--ai-chatbot-safe-right` in base al bounding box del logo e alla viewport, evitando collisioni con logo, bordi pagina ed elementi fissi principali. Su mobile il pulsante resta nella griglia della topbar e passa a riga dedicata sugli schermi stretti. Il submit AJAX usa l’endpoint `ai_chatbot.widget_ask`, protetto da login e CSRF, e riusa la stessa pipeline di contesto, motore AI, audit e sanitizzazione già usata dalla pagina completa del chatbot. La finestra è richiudibile/iconizzabile lato client senza perdere la pagina corrente.


### Rendering Markdown nel widget AI Chatbot

Il widget globale del plugin **AI Chatbot** usa un renderer Markdown client-side sicuro, definito in `app/static/app.js`, per mostrare le risposte dell'assistente direttamente nella finestra flottante. Il renderer esegue prima l'escape di HTML e caratteri speciali e poi applica solo una formattazione controllata: titoli, grassetto, corsivo, codice inline, blocchi codice, elenchi e link `http://`/`https://`. Gli stili dedicati sono in `app/static/style.css` con la classe `ai-chatbot-markdown`. La pagina completa del chatbot usa invece il filtro server-side `workflow_markdown` per mantenere coerenza di resa e sicurezza.

## AGID secure development compliance

Version 0.7.0-7 build 20260608 introduces the final hardening layer for AGID alignment: static fallback secrets were removed, production startup fails on weak secrets, sensitive configuration values are encrypted before being stored in the `Setting` table, LDAP user input is escaped before filter substitution, uploads are validated by type and content signature, destructive full imports are accepted only after archive safety checks, and the default Content-Security-Policy uses per-response nonces instead of unsafe inline scripts. Operational deployments must provide stable secret material through the environment or an orchestrator secret manager.


### Final AGID compliance closure

Version 0.7.0-7 build 20260608 now closes the remaining application-level findings: HTTP TRACE and TRACK are rejected before route dispatch and audited as `security:method_blocked`; encrypted LDAP bind credentials are decrypted before LDAP binds in login, connection tests and UID searches; `pytest.ini` makes the test suite executable from the repository root; generated `__pycache__` and `.pytest_cache` directories are excluded from the release archive. Reverse proxies should still block TRACE/TRACK as a defense-in-depth control, but compliance no longer depends only on the infrastructure layer.
...
## Hardening AGID 0.7.0-7

La versione 0.7.0-7 rafforza la conformità alle linee guida AGID per lo sviluppo sicuro:
...
La verifica completa con audit vulnerabilità dipendenze è integrata nella suite standard tramite `pip-audit` bloccante. La modalità Docker rimane disponibile con `compliance/agid/Dockerfile` e `compliance/agid/run_docker_agid_compliance.sh` per produrre evidenze isolate con rete. Entrambi i runner salvano le evidenze in `compliance/agid/<RUN_ID>/` e mantengono una sola directory risultati.
...
## Aggiornamento 0.7.0-7 - Chiusura automatica con azione e avvisi procedurali aggiornati
...
## Aggiornamento 0.7.0-7 - Knowledge base AlBot e riferimenti brochure
...
## Aggiornamento 0.7.0-7 - Configurazione form incidente, fasi procedurali e LDAP

La release 0.7.0-7 introduce la configurazione amministrativa **Layout campi incidenti**. Il setting `incident_form_default_visible_fields` stabilisce quali campi della form di nuovo incidente sono visibili per default; la ricerca nella rubrica dei destinatari esterni e la ricerca utente LDAP sono voci distinte. Il setting `incident_detail_general_visible_fields` controlla gli stessi due riquadri di ricerca nella sezione **Dati generali** del dettaglio incidente. I campi obbligatori essenziali restano sempre visibili per evitare submit non validi. Il dettaglio incidente rinomina il blocco workflow in “Fasi procedurali” e separa ogni step in un riquadro superiore descrittivo del flusso e un riquadro inferiore cliccabile relativo al task. Le label azione dispongono del flag `description_required`, migrato in modo idempotente su `config_label`, che rende obbligatoria la “Descrizione operazioni compiute”. La configurazione LDAP contiene ora filtro `{q}`, attributi di ricerca e attributi per auto-fill di Riferimento ed E-mail destinatario nelle form incidente.


## Aggiornamento 0.7.0-7 - Layout campi incidenti separato

La voce di menu amministrativa è denominata **Layout campi incidenti**. La pagina presenta due pannelli: **Nuovo incidente**, con tutte le opzioni della form di creazione, e **Dati generali incidente**, con i controlli per mostrare o nascondere i riquadri di ricerca destinatari esterni e ricerca LDAP nel dettaglio. La route LDAP per la ricerca destinatario incidente restituisce sempre JSON usando `jsonify` importato da Flask, evitando errori runtime nel dettaglio incidente.

## Aggiornamento 0.7.0-7 - LDAP multi-risultato e tipologie step workflow

La ricerca utente LDAP negli incidenti restituisce e mostra tutte le righe prodotte dalla query entro il limite applicativo. Ogni risultato espone tutti gli attributi indicati nella configurazione **Attributi di ricerca incidenti**, oltre ai dati usati per l'auto-fill di Riferimento ed E-mail destinatario. L'operatore seleziona esplicitamente la riga da usare, evitando l'uso implicito del primo risultato.

La configurazione **Layout campi incidenti** mostra per default tutte le opzioni selezionate quando non esiste una configurazione salvata. Le scelte della rubrica destinatari esterni e della ricerca LDAP restano indipendenti sia per il nuovo incidente sia per la sezione **Dati generali** dell'incidente.

Ogni `IncidentWorkflowStep` dispone del campo `step_type`, migrato in modo idempotente su `incident_workflow_step`. Le tipologie sono gestite dalla pagina **Flussi operativi incidenti**: le tipologie di default **Registrazione** ed **Esecuzione** sono sempre presenti e non eliminabili, ma la loro descrizione e modificabile; gli amministratori possono aggiungere, rinominare, descrivere o cancellare tipologie personalizzate. La configurazione e salvata nel setting JSON `workflow_step_types_json`, mantenendo anche i setting storici delle descrizioni dei default per compatibilita. Nel riquadro inferiore delle **Fasi procedurali** viene mostrata la descrizione configurata della tipologia selezionata per lo step; se una tipologia personalizzata viene eliminata, gli step associati sono riassegnati a **Registrazione**.
...
## Aggiornamento 0.7.0-7 - Gestione multi-tenant completa

La gestione multi-tenant ora distingue in modo esplicito il tenant di appartenenza dell'utente dal tenant operativo amministrato dai superuser. Gli utenti ordinari restano vincolati al proprio `tenant_id`, mentre i superuser possono scegliere un tenant attivo dalla barra superiore o dalla pagina Admin → Tenant. Il tenant attivo viene memorizzato in sessione, validato a ogni richiesta e usato da `current_tenant_id()` per tutte le configurazioni tenant-specifiche. La visibilità degli incidenti passa attraverso il tenant attivo anche per i superuser; export/import globale e funzioni amministrative dedicate restano disponibili dai percorsi espliciti.

Le configurazioni tenant-specifiche includono liste configurabili, workflow, modelli incidente, notifiche, destinatari esterni, backup, plugin e knowledge base AI. Restano condivisi tra tenant i moduli documento e relative configurazioni, HTTPS/SSL, URL applicazione e time zone applicazione. La creazione di un tenant clona la configurazione da un tenant esistente o, in assenza di scelta esplicita, dal tenant corrente/default. Il tenant `default` non è eliminabile; l'eliminazione degli altri tenant è consentita solo in assenza di utenti e incidenti associati.

## Aggiornamento 0.7.0-7 - Scomposizione route e hardening deploy

La superficie route mantiene il facade storico `app.routes` per compatibilità con plugin, test e import già esistenti, ma introduce il package `app.route_modules` per isolare progressivamente aree funzionali senza modificare endpoint o permessi. Il primo modulo centralizza la semantica multi-tenant, l'elenco delle configurazioni condivise e gli ambiti amministrativi tenant-specifici. Il deploy Kubernetes non contiene più una password PostgreSQL di esempio riutilizzabile: il Secret deve essere personalizzato prima dell'uso. La conversione LibreOffice risolve il binario tramite `shutil.which()` e mantiene il fallback PDF informativo quando il convertitore non è disponibile.

## Versione 0.7.0-7 - Isolamento tenant dei workflow e cancellazione tenant

La gestione dei flussi operativi e' ora esplicitamente tenant-scoped in tutti i percorsi amministrativi e runtime: visualizzazione, clone, import/export, salvataggio, riordino ed eliminazione usano il tenant attivo. Le righe legacy con `tenant_id` nullo vengono considerate solo come compatibilita' del tenant `default`, mentre i tenant non-default non ereditano mai flussi globali impliciti. Questo evita la comparsa di flussi spuri dopo la clonazione di un tenant.

La pagina `Admin -> Flussi operativi incidenti` include un'azione di eliminazione completa del workflow selezionato, che rimuove tutti gli step del flusso nel tenant attivo. La cancellazione tenant elimina le entita' dipendenti nell'ordine corretto, rimuovendo template incidente, step workflow e template notifica prima delle `ConfigLabel` referenziate, cosi' da evitare violazioni FK PostgreSQL.

### Spostamento incidenti tra tenant dalla home

Gli utenti superuser possono spostare un incidente verso un tenant diverso esclusivamente dalla pagina principale della lista incidenti. Ogni riga mostra un comando dedicato e distinguibile; l'elenco dei tenant di destinazione esclude il tenant sorgente ed e filtrabile digitando nella casella di ricerca. Dopo il POST di spostamento la sessione viene allineata al tenant destinazione e la lista corrente viene ricaricata.
...
## Version 0.7.0-7 - Incident list pagination and bulk actions

The main incident list is paginated by default at 20 incidents per page and exposes an in-page selector for 20, 50 or 100 incidents, with a server-side maximum of 100. The page includes multi-selection checkboxes, a select-all control for the current page, bulk tenant move for superusers and bulk delete for users with write permissions. User administration search can filter accounts by membership tenant while preserving tenant-admin visibility rules.

## Aggiornamento 0.7.0-7 - Sequence PostgreSQL nelle clonazioni tenant

La creazione di un tenant clonato e la clonazione workflow cross-tenant eseguono ora un riallineamento preventivo delle sequence PostgreSQL prima di creare record tenant-specifici come `config_label`, `person`, `recommendation`, `incident_workflow_step`, `notification_template`, `incident_template`, `external_recipient` e `backup_job`. Dopo Full import globale, il riallineamento generale viene ripetuto in una nuova transazione successiva al commit del restore, in modo che PostgreSQL veda i record appena ripristinati con ID espliciti. Questa protezione evita collisioni come `duplicate key value violates unique constraint "config_label_pkey"` nella prima clonazione tenant successiva a un restore legacy.


### Ricerca utenti per campi separati

La pagina `Admin -> Utenti` espone una ricerca strutturata per username, nome, email, backend di autenticazione, ruolo e tenant di appartenenza. Il filtro tenant interroga le membership `UserTenantRole` attive e, per gli amministratori non superuser, resta limitato al tenant attivo. Il filtro ruolo usa le membership per-tenant e conserva la compatibilita' con i campi legacy solo per backup/import storici.

### Deduplica label nella clonazione tenant
...
### Descrizione dei tipi di notifica e tooltip tag

Il modello `NotificationType` espone il campo `description`, migrato in modo idempotente sui database esistenti e valorizzato con un default sensato quando non fornito. La descrizione viene mostrata nella configurazione dei tipi di notifica e come tooltip/help nelle interfacce drag-and-drop dei tag documento, dei template e delle azioni automatiche pulsanti.

### Wizard di setup iniziale

La rotta amministrativa `GET/POST /admin/setup-wizard` implementa un wizard ri-eseguibile on-demand per il setup iniziale dei parametri principali dell’applicazione. La UI è definita in `app/templates/admin_setup_wizard.html` e mostra nome applicazione, versione, build e logo applicativo di default da `current_app.config["APP_INFO"]` e dagli asset statici, senza usare l’eventuale logo custom. La prima sezione guidata è il cambio della password dell’utente locale `admin`; le sezioni successive coprono parametri generali, logo custom, struttura e riferimenti, personale, tenant, LDAP, SSO/OAuth2, motori AI, Alfresco, documentazione, notifiche/SMTP e sicurezza/audit; ogni gruppo può essere salvato o saltato interattivamente. L’avanzamento è conservato nelle chiavi `setup_wizard_progress_json` e `setup_wizard_completed`, mentre i singoli valori vengono scritti nelle impostazioni già usate dalle pagine amministrative dedicate, evitando configurazioni parallele.
