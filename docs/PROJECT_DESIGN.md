...
La rotta amministrativa `GET/POST /admin/setup-wizard` implementa un wizard ri-eseguibile on-demand per il setup iniziale dei parametri principali dell’applicazione. La UI è definita in `app/templates/admin_setup_wizard.html` e mostra nome applicazione, versione, build e logo applicativo di default da `current_app.config["APP_INFO"]` e dagli asset statici, senza usare l’eventuale logo custom. Le sezioni guidate coprono parametri generali, logo custom, struttura e riferimenti, personale, tenant, LDAP, SSO/OAuth2, motori AI, Alfresco, documentazione, notifiche/SMTP e sicurezza/audit; ogni gruppo può essere salvato o saltato interattivamente. L’avanzamento è conservato nelle chiavi `setup_wizard_progress_json` e `setup_wizard_completed`, mentre i singoli valori vengono scritti nelle impostazioni già usate dalle pagine amministrative dedicate, evitando configurazioni parallele.
