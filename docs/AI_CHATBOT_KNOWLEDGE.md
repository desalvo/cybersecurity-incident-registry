...
Aggiornamento 0.6.0-31: nella sezione Documenti la legenda iniziale riporta nome e descrizione di ogni tag notifica; le palette drag and drop dei tag mostrano come tooltip la descrizione configurata nel tipo di notifica. La zona di rilascio e il pulsante di rimozione dei tag selezionati sono evidenziati. In Liste configurabili -> Label azioni le azioni esistenti sono sezioni verticali collassabili, chiuse per default.
