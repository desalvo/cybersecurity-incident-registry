...
On the incident detail page, ordered workflow steps show an arrow between one step and the next. The current application version is normalized to **0.2.1** and aligned across Application info, environment variables, Docker/Kubernetes and documentation.
...
Manual notification templates can optionally be linked to a PDF form template. When sending from an incident, documents generated from that form template are preselected automatically, while the operator can still add other attachments or deselect the proposed ones. If the expected document is missing, a non-blocking warning is shown. The **Admin → External recipients** address book feeds recipient/CC fields, and new emails used during sending are saved after asking for the recipient name. The address-book page includes search by name, e-mail address or notes, which is useful as the list grows.
...
## Update 0.2.1 - External recipient selection in manual notifications
...
## Update 0.2.1 - Automatic tags on generated forms
...
The application version is updated to **0.2.1**, build **2026051901**.

## Update 0.2.1 - External recipient management from Settings

Non-administrator users with the `writer` role, therefore allowed to write and edit incidents but not to access the Admin menu, can now fully manage the **External recipients** address book from **Settings → External recipients**. The page allows adding, editing, deleting and searching contacts used by manual/non-scheduled notifications with free recipient or CC fields. Administrators continue to manage the same shared address book from **Admin → External recipients**.
...
For steps based on tasks with a maximum completion time, the application shows the limit, due time and remaining time only while the step has not yet been completed; if the remaining time is less than or equal to zero, the missing step is highlighted as critical. The calculation uses the same logic already used for scheduled deadline notifications. The initial default workflow is: Initial information, Analysis, CSIRT notification, DPO notification, Privacy Authority communication, user communication and Closure. All steps, including default-workflow steps, can be added, edited or deleted from **Admin → Incident operational workflows**.

## Update 0.2.1 - Clickable workflow and procedural warnings placement
...
## Update 0.2.1 - External recipient search
...
## Update 0.2.1 - Incident templates, user search and operational fixes
...
## Update 0.2.1 - Audit sequence fix for user operations
...
## Update 0.2.1 - Destructive full import with complete database recreation
...
## Update 0.2.1 - User sequence fix
...
## Update 0.2.1 - General duplicate-key protection
...
## Update 0.2.1 - Mandatory incident reference
...
## Update 0.2.1 - Personal-data conditional workflows and disabled-user admin notification
...
## Update 0.2.1 - Workflow-step sequence fix

Incident workflow management now explicitly realigns the PostgreSQL sequence of the `incident_workflow_step` table before inserting a new task. If a residual collision occurs after Full import or restore, the insert is retried after realignment, preventing `duplicate key value violates unique constraint "incident_workflow_step_pkey"`.
